    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-slate-600 hover:text-pink-500 font-medium transition-colors relative group"
              >
                {link.name}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-xl border border-white/50 p-6 flex flex-col gap-4">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
